"""One-shot, paper-only automatic-buy watcher for v0.1."""
...
class AutoBuyManager:
    """Watch one symbol and make at most one local paper BUY order.

    This class never calls a Toss order endpoint.  It checks the moving-average
    signal every 60 seconds, performs one local PAPER order on a BUY signal,
    then stops itself.
    """
    ...
    def start(self, stock_code: str, quantity: int) -> dict[str, object]:
                ...
                if decision.signal == "BUY":
                    ...
                    with self._lock:
                        if not stop_event.is_set():
                            ...
                            self._message = "매수 신호로 모의 매수 1회를 완료했습니다."
                    return
